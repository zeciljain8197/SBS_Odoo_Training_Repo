...
class ConstructorXLSReport(models.TransientModel):
    _name = 'constructor.xls.report.wiz'
    _description = 'Constructor Excel Report'

    type = fields.Selection([('driver', 'By Driver'),
                             ('fan', 'By Fan')], 'Type')
    driver_id = fields.Many2one('formula1.driver', 'Driver')
    fan_ids = fields.Many2many('formula1.fans', string='Fan')
    ...
    def print_xls_report(self):
        ...
        cell_format = wb.add_format({'bold': True, 'center_across': True, 'font_color': 'blue', 'text_wrap': True})
        ...
        for const in consts:
            ...
            sheet.merge_range(0, 2, 0, 4, 'Constructor Report', cell_format)
            buf_image = io.BytesIO(base64.b64decode(const.img))
            sheet.insert_image('K3', "constr.png", {'image_data': buf_image, 'x_scale': x_scale, 'y_scale': y_scale})
